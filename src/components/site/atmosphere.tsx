...
export function Atmosphere({ className, variant = "hero", intensity = 1 }: Props) {
  return (
    ...
    >
      ...
      <div
        className={cn(
          "absolute rounded-full border border-accent/20",
          variant === "hero"
            ? "-right-40 top-24 h-[38rem] w-[38rem]"
            : "-right-56 top-10 h-[28rem] w-[28rem]",
        )}
      />
      <div
        className={cn(
          "absolute rounded-full border border-accent/10",
          variant === "hero"
            ? "-right-24 top-40 h-[30rem] w-[30rem]"
            : "-right-36 top-24 h-[22rem] w-[22rem]",
        )}
      />
    </div>
  );
